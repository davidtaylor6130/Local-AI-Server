...
static bool process_task(const Task& t) {
    try {
        ...
        std::cout << "[bulk-asset-organizer] Path: " << j.value("path", std::string("/pool/assets")) << std::endl;
    } catch (...) {
        std::cout << "[bulk-asset-organizer] Processing job " << t.id << std::endl;
    }
    // TODO: compute CLIP embeddings and tag assets
    return true;
}

int main(int argc, char** argv) {
    const std::string agent = "bulk-asset-organizer";
    ...
    std::cout << "[bulk-asset-organizer] Starting. QUEUE_URL=" << queue_url << std::endl;
    do {
