...
static bool process_task(const Task& t) {
    try {
        ...
        std::cout << "[announcements] Preparing notes for tag: " << j.value("tag", std::string("(none)")) << std::endl;
    } catch (...) {
        std::cout << "[announcements] Processing job " << t.id << std::endl;
    }
    // TODO: summarize commits and draft announcements
    return true;
}

int main(int argc, char** argv) {
    const std::string agent = "announcements";
    ...
    std::cout << "[announcements] Starting. QUEUE_URL=" << queue_url << std::endl;
    do {
