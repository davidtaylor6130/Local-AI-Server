...
static bool process_task(const Task& t) {
    try {
        ...
        std::cout << "[daily-reflection] Date: " << j.value("date", std::string("today")) << std::endl;
    } catch (...) {
        std::cout << "[daily-reflection] Processing job " << t.id << std::endl;
    }
    // TODO: summarize commits/notes and draft reflection
    return true;
}

int main(int argc, char** argv) {
    const std::string agent = "daily-reflection";
    ...
    std::cout << "[daily-reflection] Starting. QUEUE_URL=" << queue_url << std::endl;
    do {
