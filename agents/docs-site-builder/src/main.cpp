...
static bool process_task(const Task& t) {
    try {
        ...
        std::cout << "[docs-site-builder] Framework: " << j.value("framework", std::string("mkdocs")) << std::endl;
    } catch (...) {
        std::cout << "[docs-site-builder] Processing job " << t.id << std::endl;
    }
    // TODO: assemble docs and produce static site
    return true;
}

int main(int argc, char** argv) {
    const std::string agent = "docs-site-builder";
    ...
    std::cout << "[docs-site-builder] Starting. QUEUE_URL=" << queue_url << std::endl;
    do {
