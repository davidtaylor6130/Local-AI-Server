...
static bool process_task(const Task& t) {
    try {
        ...
        std::cout << "[ticket-triage] Subject: " << j.value("subject", std::string("(no subject)")) << std::endl;
    } catch (...) {
        std::cout << "[ticket-triage] Processing job " << t.id << std::endl;
    }
    // TODO: classify and draft reply
    return true;
}

int main(int argc, char** argv) {
    const std::string agent = "ticket-triage";
    ...
    std::cout << "[ticket-triage] Starting. QUEUE_URL=" << queue_url << std::endl;
    do {
