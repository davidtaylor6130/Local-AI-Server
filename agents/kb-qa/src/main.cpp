...
static bool process_task(const Task& t) {
    try {
        ...
        std::cout << "[kb-qa] Question: " << j.value("question", std::string("(none)")) << std::endl;
    } catch (...) {
        std::cout << "[kb-qa] Processing job " << t.id << std::endl;
    }
    // TODO: perform retrieval and answer with citations
    return true;
}

int main(int argc, char** argv) {
    const std::string agent = "kb-qa";
    ...
    std::cout << "[kb-qa] Starting. QUEUE_URL=" << queue_url << std::endl;
    do {
